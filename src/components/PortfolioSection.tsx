      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <ScrollAnimation variant="fadeUp" delay={0.2} className="flex flex-wrap gap-3 mb-12">
          {filters.map((filter) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all duration-300 hover:scale-105 active:scale-95 ${
                activeFilter === filter
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item, index) => (
            <div
              key={item.id}
              className={`transition-all duration-500 ${index === 0 || index === 3 ? 'md:row-span-2' : ''}`}
